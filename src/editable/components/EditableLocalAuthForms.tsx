...
const buttonClass =
  'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--slot4-page-text)] px-6 text-sm font-semibold text-[var(--slot4-cream)] transition duration-500 hover:bg-[var(--slot4-accent)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[18px] px-5 py-3.5 text-sm font-medium ${status === 'success' ? 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-page-text)]' : 'border border-[var(--slot4-accent)] text-[var(--slot4-accent)]'}`}>{message}</p> : null}
